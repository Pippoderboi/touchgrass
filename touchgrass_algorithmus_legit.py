...
class EmpfehlungsAlgorithmus:
    ...
    def __init__(self):
        ...
        self.kategorien_scores = {
            ...
            'draussen': {
                ...
                'outdoor_sports': {'aktiv': 5, 'entspannt': 1, 'kulturell': 1, 'ruhe': 0, 'gesellschaft': 4},
            }
        }
    ...
    def _frage_zurueck_hauptmenu(self) -> bool:
        """
        Fragt ob zurück zum Hauptmenü oder beenden
        """
        print(f"\n{'='*60}")
        zurueck = input("🔄 Zurück zum Hauptmenü? (j/n): ").lower().strip()
        return zurueck.startswith('j')
    ...
    def empfehlungen_anzeigen(self, empfehlungen: List[str], debug_info: List[Empfehlung] = None):
        ...
        # Debug-Info wenn verfügbar - KORRIGIERT!
        if debug_info:
            print(f"\n🔍 DEBUG-INFO (alle bewerteten Kategorien):")
            for emp in sorted(debug_info, key=lambda x: x.score, reverse=True):
                # Verwende den aktuellen min_score für die Anzeige
                status = "✅" if emp.score >= self.min_score else "❌"
